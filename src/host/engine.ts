...
export class LifecycleEngine {
  ...
  constructor(
    private readonly loaderBaseUrl: string | undefined,
  ...
  private async runToggle(
    ...
    try {
      ...
      await this.awaitEffectiveState(binding.entryId, disable)
        .catch((error: unknown) => {
          if ((error as { code?: string }).code === 'BLOCKED_BY_ANCESTOR') {
            state.keepRow = true
          }
          throw error
        })
  ...
  private async awaitEffectiveState(entryId: string, disabled: boolean): Promise<void> {
    ...
    for (;;) {
      ...
      if (Date.now() >= deadline) {
        const options = entry.options as { disabled?: unknown }
