...
import { applyManagedToggleRows, readManagedToggleRows, removeManualInsertRows, } from './patch-editor.ts';
...
export async function runUninstallTransaction(options) {
    ...
    try {
        ...
        await options.withPatchLock(async () => {
            ...
            for (const id of options.affectedEntryIds)
                rows.push({ entryId: id, disabled: true });
