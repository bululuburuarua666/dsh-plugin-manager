...
import {
  originOverrideEntrySchema,
  originOverrideFileSchema,
  parseOriginOverrides,
  type PluginOriginOverrideEntry,
} from './origin.ts'
import { managerFailure } from './failure.ts'
import { normalizeInventoryCardText } from './protocol.ts'
...
export class OriginStore {
  ...
  revision(): string {
    if (this.profileDir === null) return ORIGIN_MISSING_REVISION
    try {
  ...
  async update(
    packageName: string,
    override: PluginOriginOverrideEntry | null,
    expectedRevision: string,
  ): Promise<{ readonly revision: string }> {
    const path = this.originsPath()
    let validated: PluginOriginOverrideEntry | null = null
    if (override !== null) {
    ...
    return withFileLock(path, async () => {
      ...
      if (text !== null) {
        ...
        for (const [name, value] of existing.overrides.packages) packages.set(name, value)
      }
      ...
      if (validated === null) packages.delete(packageName)
      else packages.set(packageName, validated)
