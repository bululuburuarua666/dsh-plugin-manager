...
import { InventoryAssembler, profileDirOf } from "./host/inventory.js";
import { OriginStore } from "./host/origin-store.js";
...
import { ManagerFailure, managerFailure } from "./host/failure.js";
...
function rosterRows(iterable) {
    ...
    for (const entry of iterable) {
        ...
        if (options.group !== undefined && options.group !== null && options.group !== false)
            continue;
        if (entry.subtree !== undefined || entry.subgroup !== undefined)
            continue;
...
export default {
    ...
    apply(ctx) {
        ...
        ctx.inject(['connection'], connectionCtx => {
            ...
            const dispose = rpc.handle(MANAGER_CHANNEL, (endpoint, payload, signal) => managerHandler(engine, inventory, origins, entries, persistence, endpoint, payload, signal, connectionCtx.logger), { authority: 'loopback' });
 ...
 */
async function managerHandler(engine, inventory, origins, entries, persistence, endpoint, payload, signal, ctxLogger) {
    try {
        ...
        switch (endpoint) {
            ...
            case 'originState': {
                const description = describeRequestOrigin(inventory, entries, request.value.entryId);
                return {
                    ok: true,
                    value: originStateValue(request.value.entryId, description, origins.revision()),
                };
            }
            case 'originUpdate': {
                ...
                const description = describeRequestOrigin(inventory, entries, request.value.entryId);
                await origins.update(description.packageName, request.value.override, request.value.expectedOriginRevision);
                // Re-read AFTER the commit so the response reflects the stored state.
                const fresh = describeRequestOrigin(inventory, entries, request.value.entryId);
                return {
                    ok: true,
                    value: originStateValue(request.value.entryId, fresh, origins.revision()),
                };
            }
        }
    }
 ...
 */
function describeRequestOrigin(inventory, entries, entryId) {
...
function originStateValue(entryId, description, originRevision) {
    return {
        protocolVersion: PROTOCOL_VERSION,
        entryId,
