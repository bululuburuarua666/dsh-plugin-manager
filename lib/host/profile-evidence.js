 ...
 */
export function engineTreeRootOf() {
 ...
 */
function scanRootInto(root, index) {
    let entries;
    try {
    ...
    for (const first of entries) {
        ...
        if (first.name.startsWith('@')) {
            ...
            for (const second of scoped) {
                ...
                if (!index.has(name))
                    index.set(name, join(root, first.name, second.name));
            }
        }
...
export function createEvidenceSession(profileDir, manifest, patchText, engineTreeRoot) {
    return {
        profileDir,
        manifest,
        patchText,
...
function cachedRealpath(session, path) {
