...
export function resolvePnpmEntry(env = process.env) {
...
export async function runPnpm(args, cwd, env = process.env, timeoutMs = PACKAGE_MANAGER_TIMEOUT_MS) {
    ...
    await new Promise((resolveRun, rejectRun) => {
        execFile(process.execPath, [entry, ...args], { encoding: 'utf8', signal, windowsHide: true, cwd }, (error) => {
            if (error !== null) {
