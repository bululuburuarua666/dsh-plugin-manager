 ...
 */
import { z } from 'zod';
import { normalizeInventoryCardText } from "./card.js";
...
export const OFFICIAL_REPOSITORY_URLS = [
 ...
 */
export function isOfficialCandidate(evidence) {
    if (!evidence.packageName.startsWith('@deepseek-ai/'))
        return false;
    if (evidence.resolutionRoot === 'engine' || evidence.insideEngineCheckout)
        return true;
 ...
 */
export function parseFileSpecifierTarget(specifier) {
 ...
 */
export function stripPeerSuffix(resolution) {
 ...
 */
export function parseOriginOverrides(text) {
    let raw;
    try {
        raw = JSON.parse(text);
    }
    ...
    const diagnostics = [];
    for (const [name, value] of Object.entries(record.packages)) {
 ...
 */
export function parseManifestOrigin(value) {
...
function normalizeNote(value) {
    ...
    if (text === null)
        return null;
    const cap = (input) => input.length <= NOTE_MAX ? input : input.slice(0, NOTE_MAX - 1).trimEnd();
 ...
 */
export function normalizeOrigin(input, declaredBy) {
 ...
 */
export function mergeOriginOverride(base, override) {
 ...
 */
export function heuristicOrigin(evidence) {
    const diagnostics = [];
    let kind;
 ...
 */
export function resolveOrigin(evidence, layers) {
    const diagnostics = [];
    let base = null;
